...
const sendShowReminders = inngest.createFunction(
  ...
  async ({ step }) => {
    const now = new Date();
    const in8Hours = new Date(now.getTime() + 8 * 60 * 60 * 1000); // now + 8 hours
    const windowStart = new Date(in8Hours.getTime() - 10 * 60 * 1000); 
    //Creates a 10-minute window before the 8-hour mark.
    //So if in8Hours is 8:00 PM → windowStart is 7:50 PM.This prevents duplicate reminders and ensures only the shows starting around that time are included.
    ...
    ("prepare reminder tasks", async () => {
      ...
      for (const show of shows) {
        ...
        const userIds=[...new setInterval(Object.values(show.occupiedSeats))];

        if(userIds.length===0) continue;

        const users = await User.find({_id:{$in:userIds}}).selec("name email");
